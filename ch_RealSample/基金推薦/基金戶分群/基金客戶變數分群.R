...
fund_datasets <- sqlQuery(conn,"select * from Test.dbo.v_Shane_基金客戶分群_基礎",stringsAsFactors=F)
...
system.time(gower_distance <- daisy(mfunds[c(1:5000),c(1,2,3,19,20,23,25)],metric = "gower"))

summary(gower_distance)

gower_mat <-  gower_distance %>% as.matrix() 

gower_mat[1:5,1:5] # check 
### 儲存 gower_distance, 
## fund_datasets==> 原始資料,funds ==> 填補空值,mfund ==> 計算gower_distance用
save(fund_datasets,mfunds,gower_distance,file="customerDistance.RData")

# Output most similar pair
fund_datasets[
  which(gower_mat == min(gower_mat[gower_mat != min(gower_mat,na.rm=T)],na.rm=T),arr.ind = TRUE)[1, ], ] %>% glimpse()

# most dis-similar pair
funds[  which(gower_mat == max(gower_mat[gower_mat != max(gower_mat,na.rm=T)],na.rm=T),arr.ind = TRUE)[1, ], ] %>% glimpse()
